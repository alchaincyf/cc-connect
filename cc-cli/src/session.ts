...
function getMessageKey(content: string, type: string): string {
  return `${type}:${content.substring(0, 100)}`;
}

function isDuplicate(content: string, type: string): boolean {
...
const recentUserInputs: Set<string> = new Set();
...
function processFallbackOutput(): void {
  ...
  if (claudeMatch) {
    ...
    if (content.length > 20 && !isDuplicate(content, 'claude')) {
      ...
      if (!/^\w+\(/.test(content)) {
        // 过滤思考状态词汇
        const startsWithThinking = THINKING_KEYWORDS.some(keyword =>
...
function handleHookEvent(event: ProcessedEvent): void {
  ...
  if (event.message) {
    ...
    const parsedMessage: ParsedMessage = {
      ...
      options: msg.options as any,
      tool: msg.tool as any,
    };
...
function sendToPhone(message: ParsedMessage): void {
  state.wsClient?.send({
    type: 'message',
    message,
  });
