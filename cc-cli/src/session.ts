...
import { parseOutput, flushBuffer, resetParser, ParsedMessage } from './parser';
...
function isDuplicateMessage(msg: ParsedMessage): boolean {
...
export async function startSession(options: SessionOptions): Promise<void> {
  ...
  try {
    state.wsClient = new WebSocketClient(options.server, state.id, state.secret);
...
function displayQRCode(): void {
  ...
  qrcode.generate(pairingCode, { small: true }, (code: string) => {
    console.log(code);
  });
...
// 完整移除所有终端控制序列和特殊字符
function stripTerminalSequences(str: string): string {
  let result = str;
...
function flushOutputBuffer(): void {
  if (!rawOutputBuffer || !state.wsClient?.isConnected) return;
  ...
  resetParser();
  const messages = parseOutput(bufferToProcess);
  ...
  // 过滤和分类消息
  const messagesToSend: ParsedMessage[] = [];
  let latestStatus: ParsedMessage | null = null;
  ...
  for (const msg of allMessages) {
    ...
    // 过滤残缺的思考状态
    if (/thinking\)?|thought for/i.test(msg.content) && msg.content.length < 30) continue;
    ...
    // raw 类型：只有内容有意义才发送
      ...
      // 过滤明显的噪音
      if (/^[a-z]{2,4}↵/i.test(msg.content)) continue; // 如 "tin↵", "dul↵"
      if (/·\s*thinking/i.test(msg.content)) continue;
