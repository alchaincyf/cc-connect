...
function parseLine(line: string): ParsedMessage | null {
  ...
  if (/^(ought\s+)?for\s+\d+s\)?$/i.test(trimmed)) return null;
  ...
  if (/^[⏺●]/.test(trimmed)) {
    ...
    if (/^Welcome\s+to\s+Claude\s+Code/i.test(content)) return null;
    // 过滤默认AI助手欢迎消息
    if (/你好.*我是你的AI助手.*有什么可以帮助你的/i.test(content)) return null;
    if (/Hello.*I'm\s+Claude.*How\s+can\s+I\s+(help|assist)/i.test(content)) return null;
  ...
  // 过滤 IDE 状态消息和系统提示
  if (/^[◯○]\s*(IDE|VSCode|MCP|\/ide)/i.test(trimmed)) return null;
  // 过滤欢迎消息
  if (/^[✻✽✶✳]\s*Welcome\s+to\s+Claude\s+Code/i.test(trimmed)) return null;
  if (/^\/help\s+for\s+help/i.test(trimmed)) return null;
  if (/^\/status\s+for\s+your\s+current\s+setup/i.test(trimmed)) return null;
  // 过滤工作目录提示
  if (/^cwd:\s*\//i.test(trimmed)) return null;

  // 6.1 过滤帮助提示和系统消息
  // 这些不是权限请求，只是 Claude Code 的界面提示
  // 匹配多种格式：`? for shortcuts`, `? shortcuts`, `?for shortcuts` 等
  if (/^\??\s*(for\s+)?(shortcuts|help|commands)/i.test(trimmed)) return null;
  // 过滤单独的 `?` 符号或 `? ` 后跟提示
  if (/^\?\s*$/.test(trimmed)) return null;
  // 过滤 IDE 连接状态提示（出现在底部状态栏）
  if (/^[◯○]\s*(IDE|VSCode|MCP)\s*(connected|disconnected|connecting)/i.test(trimmed)) return null;
  // 过滤自动更新失败提示
  if (/^[✗×]\s*Auto-update\s+failed/i.test(trimmed)) return null;
  if (/Try\s+(claude\s+doctor|npm\s+i\s+-g)/i.test(trimmed)) return null;
  ...
  // - "Permission denied" 等
  // 注意排除系统状态提示
  const isRealPermissionRequest =
    (/^(\?\s*)?(Allow|Deny|Permission)/i.test(trimmed) ||
     /^⚠️/.test(trimmed) ||
     /^🔐/.test(trimmed) ||
  ...
  const isSelectionQuestion =
    /Do you want to (proceed|continue)\?/i.test(trimmed) ||
  ...
  // 额外过滤：底部状态栏的系统消息
  if (/IDE\s*(dis)?connected|Auto-update\s+failed/i.test(trimmed)) return null;
  // 过滤带时间戳或分隔符的系统消息
  if (/^\d{4}-\d{2}-\d{2}|^[-=]{3,}|^[╭╰─]+$/i.test(trimmed)) return null;
